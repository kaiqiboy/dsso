...
if __name__ == "__main__":
    ...
    parser.add_argument("-m", type=str, nargs='?', default="./imdb/model-imdb.pt") # model weights
    parser.add_argument("-d", type=str, nargs='?', default="./imdb") # metadata dir
    ...
    seq_len = 57
    col_len = (len(pickle.load(open(d+'column_onehot_imdb.pkl', 'rb'))) + 5) * 5 # 5 metadata info, an operator relates to at most 5 columns
    condition_len = 20
    table_len = len(pickle.load(open(d+'table_onehot_imdb.pkl', 'rb'))) + 2 # 2 metadata info
    operation_len = len(pickle.load(open(d+'operation_onehot_imdb.pkl', 'rb')))
    lstm_input_dim = seq_len + col_len + condition_len + table_len + operation_len

    preprocessor = Preprocessor(d+'column_onehot_imdb.pkl', \
        d+'table_onehot_imdb.pkl', \
        d+'operation_onehot_imdb.pkl',\
        d+"imdb-metadata.json",\
        max_resource,\
        d+"usif_imdb",\
