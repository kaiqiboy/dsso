  ...
  class TpchDataGenerator(dir: String) extends DataGenerator {
    ...
    val inputDir: String = if (dir.endsWith("/")) dir else dir + "/"
      ...
      val dfMap: Map[String, DataFrame] = Map(
          "customer" -> spark.read.option("delimiter", "|").schema(customer_schema).csv(inputDir + "customer/customer.tbl*"),
          "lineitem" -> spark.read.option("delimiter", "|").schema(lineitem_schema).csv(inputDir + "lineitem/lineitem.tbl*"),
          "nation" -> spark.read.option("delimiter", "|").schema(nation_schema).csv(inputDir + "nation/nation.tbl*"),
          "region" -> spark.read.option("delimiter", "|").schema(region_schema).csv(inputDir + "region/region.tbl*"),
          "orders" -> spark.read.option("delimiter", "|").schema(order_schema).csv(inputDir + "orders/orders.tbl*"),
          "part" -> spark.read.option("delimiter", "|").schema(part_schema).csv(inputDir + "part/part.tbl*"),
          "partsupp" -> spark.read.option("delimiter", "|").schema(partsupp_schema).csv(inputDir + "partsupp/partsupp.tbl*"),
          "supplier" -> spark.read.option("delimiter", "|").schema(supplier_schema).csv(inputDir + "supplier/supplier.tbl*")
          )
