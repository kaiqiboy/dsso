...
object SQLConf {
  ...
  // for dl-based cost estimation optimization
  val CEO = buildConf("spark.sql.ceo")
    .internal()
    .doc("Enable cost estimation optimization or not.")
    .version("3.2.0")
    .booleanConf
    .createWithDefault(false)

  val CEO_DIR = buildConf("spark.sql.ceoDir")
    .internal()
    .doc("Direction to the cost estimation optimization supporting files.")
    .version("3.2.0")
    .stringConf
    .createWithDefault("/")

  val CEO_SERVER_IP = buildConf("spark.sql.ceoServerIP")
    .internal()
    .doc("The IP address for CEO python server.")
    .version("3.2.0")
    .stringConf
    .createWithDefault("127.0.0.1")

  val CEO_SERVER_PORT = buildConf("spark.sql.ceoServerPort")
    .internal()
    .doc("The port number for CEO python server.")
    .version("3.2.0")
    .intConf
    .createWithDefault(8308)

  val CEO_LENGTH_THRESHOLD = buildConf("spark.sql.ceoLengthThreshold")
    .internal()
    .doc("Plan length threshold to determine applying ceo or not.")
    .version("3.2.0")
    .intConf
    .createWithDefault(500)

  val CEO_PRUNE_AGGRESSIVE = buildConf("spark.sql.ceoPruneAggressive")
    .internal()
    .doc("Prune aggressively during plan exploration or not.")
    .version("3.2.0")
    .booleanConf
    .createWithDefault(false)

  val CEO_METADATA_DIR = buildConf("spark.sql.ceoMetadataDir")
    .internal()
    .doc("The directory to the metadata file for plan pruning.")
    .version("3.2.0")
    .stringConf
    .createWithDefault("")

  val CEO_SMALL_TABLE_THRESHOLD = buildConf("spark.sql.ceoSmallTableThreshold")
    .internal()
    .doc("Force broadcasting without exploration threshold, default 10MB.")
    .version("3.2.0")
    .longConf
    .createWithDefault(10485760)
