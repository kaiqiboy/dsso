...
import org.apache.spark.sql.internal.SQLConf.{CEO, CEO_DIR, CEO_METADATA_DIR, CEO_PRUNE_AGGRESSIVE, CEO_SERVER_IP, CEO_SERVER_PORT, CEO_SMALL_TABLE_THRESHOLD}
import org.apache.spark.sql.internal.StaticSQLConf.CATALOG_IMPLEMENTATION
import org.apache.spark.sql.sources.BaseRelation
import org.apache.spark.sql.streaming._
import org.apache.spark.sql.types.{DataType, DoubleType, IntegerType, LongType, MapType, StringType, StructType}
...
class SparkSession private(
  ...
  var serverProcess: Option[Runtime] = None
  ...
  def stop(): Unit = {
    sparkContext.stop()
    if (this.serverProcess.isDefined) this.serverProcess.get.exit(0)
  }
  ...
  def startCeoServer(): Runtime = {
    val rt = Runtime.getRuntime()
    val ceoDir = conf.get(CEO_DIR)
    val serverDir = if (ceoDir.nonEmpty && ceoDir.last == '/') ceoDir.dropRight(1)
    else ceoDir
    val serverIP = conf.get(CEO_SERVER_IP)
    val serverPort = conf.get(CEO_SERVER_PORT)
    try {
      new Socket(InetAddress.getByName(serverIP), serverPort) // first check if the server is already up
    }
    catch {
      case _: Exception =>
        if (InetAddress.getByName(serverIP).getHostAddress == InetAddress.getLocalHost.getHostAddress ||
          InetAddress.getByName(serverIP).isLoopbackAddress) {
          val command = s"python $serverDir/server.py -ip $serverIP -p $serverPort"
          rt.exec(command)
        }
    }
    rt
  ...
  def parseTableMetadata(): Unit = {
    val colSchema = new StructType().add("numerical", IntegerType, false)
      .add("minValue", DoubleType, false)
      .add("maxValue", DoubleType, false)
      .add("nulls", LongType, false)
      .add("distinct", LongType, false)
    val schema = new StructType().add("name", StringType, false)
      .add("length", LongType, false)
      .add("size", LongType, false)
      .add("columns", MapType(StringType, colSchema))

    import this.implicits._
    val tablesInfos = this.read.schema(schema)
      .json(this.conf.get(CEO_METADATA_DIR)).as[TableInfo].collect()
    this.catalog.tableMetadata ++= tablesInfos
  }

  def findSmallColumns(): Unit = {
    if (conf.get(CEO_PRUNE_AGGRESSIVE)) {
      val columnsMap = sessionState.catalog.tempViews.
        mapValues(_.toString.split("Relation \\[")(1).split("\\]").head.split(","))
      val smallTables = catalog.tableMetadata.filter(_.size < conf.get(CEO_SMALL_TABLE_THRESHOLD)).
        map(_.name)
      //      // scalastyle:off println
      //      println(columnsMap)
      //      println(smallTables.deep)
      //      println(columnsMap.filterKeys(smallTables.contains).values.flatten.toArray.deep)
      //      // scalastyle:on println
      catalog.smallColumns ++= columnsMap.filterKeys(smallTables.contains).values.flatten.toSet
    }
  }
...
object SparkSession extends Logging {
  ...
  class Builder extends Logging {
    ...
    def getOrCreate(): SparkSession = synchronized {
      ...
      if (sparkConf.get(CEO)) {
        // scalastyle:off println
        println(s"--- Starting python server for cost prediction at ${sparkConf.get(CEO_SERVER_IP)}:${sparkConf.get(CEO_SERVER_PORT)}")
        // scalastyle:off println
        session.serverProcess = Some(session.startCeoServer())

        if (sparkConf.get(CEO_PRUNE_AGGRESSIVE)) {
          try {
            session.parseTableMetadata()
            session.findSmallColumns()
          }
          catch {
            case _: Throwable =>
              throw new IllegalArgumentException(s"Fail to load table metadata from ${sparkConf.get(CEO_METADATA_DIR)}")
          }
        }
      }
      return session
