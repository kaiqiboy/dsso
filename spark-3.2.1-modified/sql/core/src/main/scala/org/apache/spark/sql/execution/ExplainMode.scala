...
case object PhysicalPlansMode extends ExplainMode { val name = "physicalplans" }

case object PhysicalPlansFormattedMode extends ExplainMode { val name = "physicalplansformatted" }
...
object ExplainMode {
  ...
  def fromString(mode: String): ExplainMode = mode.toLowerCase(Locale.ROOT) match {
    ...
    case PhysicalPlansMode.name => PhysicalPlansMode
    case PhysicalPlansFormattedMode.name => PhysicalPlansFormattedMode
    case _ => throw new IllegalArgumentException(s"Unknown explain mode: $mode. Accepted " +
      "explain modes are 'simple', 'extended', 'codegen', 'cost', 'formatted', 'physicalplans', 'physicalplansformatted'.")
  }
}
