...
import org.apache.spark.sql.execution.joins.{BaseJoinExec, BroadcastNestedLoopJoinExec, CartesianProductExec, ShuffledHashJoinExec}
...
class SparkPlanner(val session: SparkSession, val experimentalMethods: ExperimentalMethods)
  ...
  override def collectPlaceholders(plan: SparkPlan): Seq[(SparkPlan, LogicalPlan)] = {
    plan.collect {
      case placeholder@PlanLater(logicalPlan) => placeholder -> logicalPlan
    }
  }

  override def prunePlans(plans: Seq[SparkPlan]): Seq[SparkPlan] = {
    // TODO: We will need to prune bad plans when we improve plan space exploration
    //       to prevent combinatorial explosion.

    //    // scalastyle:off println
    //    println(plans.map(_.nodeName))
    //    // scalastyle:on println

    // Prune bad joins
    if (plans.length > 1 && plans.head.isInstanceOf[BaseJoinExec]) {
      val catB = if (session.conf.get(CEO_PRUNE_AGGRESSIVE)) {
        plans.filter(x => x.isInstanceOf[CartesianProductExec] || x.isInstanceOf[BroadcastNestedLoopJoinExec] || x.isInstanceOf[ShuffledHashJoinExec])
      }
      else plans.filter(x => x.isInstanceOf[CartesianProductExec] || x.isInstanceOf[BroadcastNestedLoopJoinExec])
      val catA = plans.diff(catB)
      if (catA.length > 1) catA
      else plans
    }
    else plans
