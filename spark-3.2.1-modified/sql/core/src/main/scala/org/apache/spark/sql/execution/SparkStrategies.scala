...
import org.apache.spark.sql.internal.SQLConf.CEO_PRUNE_AGGRESSIVE
...
abstract class SparkStrategy extends GenericStrategy[SparkPlan] {
  ...
  override def planLater(plan: LogicalPlan): SparkPlan = PlanLater(plan)
}
...
abstract class SparkStrategies extends QueryPlanner[SparkPlan] {
  ...
  override def plan(plan: LogicalPlan): Seq[SparkPlan] = {
  ...
  override def planFallback(plan: LogicalPlan): Seq[SparkPlan] = {
    super.planFallback(plan).map { p =>
      val logicalPlan = plan match {
        case ReturnAnswer(rootPlan) => rootPlan
        case _ => plan
      }
  ...
  object JoinSelection extends Strategy
    ...
    def apply(plan: LogicalPlan): Seq[SparkPlan] = plan match {
      ...
      case j@ExtractEquiJoinKeys(joinType, leftKeys, rightKeys, nonEquiCond, left, right, hint) =>
        ...
        //        println("=== debug1")
        //        println(plan)
        //        (Seq(createBroadcastHashJoin(false),
        //          createSortMergeJoin(), createShuffleHashJoin(false), createCartesianProduct())
        //          .filter(_.isDefined).map(_.get.head) ++ createJoinWithoutHint()).reverse.foreach(println)
        ...
        //        println(left)
        //        println(right)
        //        println((leftKeys ++ rightKeys).map(_.toString).toSet)
        //        println(SparkSession.getActiveSession.get.catalog.smallColumns)
        //        // scalastyle:on println

        if (session.conf.get(CEO_PRUNE_AGGRESSIVE) &&
          ((leftKeys ++ rightKeys).map(_.toString).toSet intersect SparkSession.getActiveSession.get.catalog.smallColumns).nonEmpty) {
          Seq(createBroadcastHashJoin(false)).filter(_.isDefined).map(_.get.head) ++ createJoinWithoutHint()
        }
        else {
          Seq(createBroadcastHashJoin(false),
            createSortMergeJoin(), createShuffleHashJoin(false), createCartesianProduct())
            .filter(_.isDefined).map(_.get.head) ++ createJoinWithoutHint()
        }
      ...
      case j@ExtractSingleColumnNullAwareAntiJoin(leftKeys, rightKeys) =>
        ...
        //          None, planLater(j.left), planLater(j.right), isNullAwareAntiJoin = true)).foreach(println)
        //        // scalastyle:on println
      ...
      case logical.Join(left, right, joinType, condition, hint) =>
        ...
        //        println("=== debug3")
        //        println(plan)
        //        (Seq(createBroadcastNLJoin(hintToBroadcastLeft(hint), hintToBroadcastRight(hint)),
        //          createCartesianProduct()).filter(_.isDefined)
        //          .map(_.get.head) ++ createJoinWithoutHint()).foreach(println)
        //        // scalastyle:on println

        Seq(createBroadcastNLJoin(hintToBroadcastLeft(hint), hintToBroadcastRight(hint)),
          createCartesianProduct()).filter(_.isDefined)
          .map(_.get.head) ++ createJoinWithoutHint()
