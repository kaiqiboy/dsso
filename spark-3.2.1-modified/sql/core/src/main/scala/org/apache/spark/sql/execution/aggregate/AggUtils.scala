...
object AggUtils {
  ...
  private def createAggregates(
    ...
    var res = Seq[SparkPlan]()
    val useHash = HashAggregateExec.supportsAggregate(
      aggregateExpressions.flatMap(_.aggregateFunction.aggBufferAttributes))
    if (useHash) {
      res = res :+ HashAggregateExec(
        requiredChildDistributionExpressions = requiredChildDistributionExpressions,
        groupingExpressions = groupingExpressions,
        aggregateExpressions = mayRemoveAggFilters(aggregateExpressions),
        aggregateAttributes = aggregateAttributes,
    ...
    if (objectHashEnabled && useObjectHash) {
      res = res :+ ObjectHashAggregateExec(
        requiredChildDistributionExpressions = requiredChildDistributionExpressions,
        groupingExpressions = groupingExpressions,
        aggregateExpressions = mayRemoveAggFilters(aggregateExpressions),
        aggregateAttributes = aggregateAttributes,
  ...
  def planAggregateWithoutDistinct(
    ...
    val partialAggregates = createAggregates(
    ...
    val interExecs: Seq[SparkPlan] = partialAggregates.map(partialAggregate =>
    ...
    val finalAggregates = interExecs.flatMap(interExec => createAggregates(
  ...
  def planAggregateWithOneDistinct(
    ...
    // 1. Create an Aggregate Operator for partial aggregations.
    val partialAggregates: Seq[SparkPlan] = {
    ...
    val partialMergeAggregates: Seq[SparkPlan] = {
      val aggregateExpressions = functionsWithoutDistinct.map(_.copy(mode = PartialMerge))
      val aggregateAttributes = aggregateExpressions.map(_.resultAttribute)
      partialAggregates.flatMap(partialAggregate => createAggregates(
    ...
    val partialDistinctAggregates: Seq[SparkPlan] = {
      ...
      partialMergeAggregates.flatMap(partialMergeAggregate => createAggregates(
        groupingExpressions = groupingAttributes,
    ...
    val finalAndCompleteAggregates: Seq[SparkPlan] = {
      ...
      partialDistinctAggregates.flatMap(partialDistinctAggregate => createAggregates(
        requiredChildDistributionExpressions = Some(groupingAttributes),
        groupingExpressions = groupingAttributes,
